...
| Hybrid FTS5 + vector + entity + graph-seed recall, per-arm capped, RRF-fused, composite re-ranked (issue #136) | `skills/memory/scripts/storelib/recall.py` `recall_memory` / `_recall_one_tier` / `_rrf_fuse`; `storelib/links.py` `graph_seed_ids`; pinned by `tests/test_mmr.py`, `tests/test_hybrid_default.py`, `tests/test_graph_seed_arm.py` | shipped |
...
| Decision-point eval bucket: #85-shaped intent prose + prior-turn operation context (`ops` gold field) retrieves the hazard lessons WITH ops and misses without; composed via the SAME shared function the hook uses; still non-CI-gating | `eval/gold.jsonl` items `decision-*`; `tests/fixtures/eval_store.py` rowids 65–70; `storelib/eval_gold.py` `ops` field + compose; `storelib/ops_tokens.py` `compose_inject_query`; `tests/test_ops_tokens.py` (hit-with-ops AND miss-prose-only); `tests/test_eval_runner.py` (rank pins) | shipped (issue #88, #85 direction 4) |
...
| Counterfactual replay (`scripts/eval_counterfactual.py`): recorded five-session A/B with `ZMEM_INJECT` 1/0 through the real passive lane, pinned `recorded-stub-v1` path, report-only (no counters, read-only store URI, SHA-verified), real models gated behind `--allow-model-calls` with the exact skip line; record-only in CI | `scripts/eval_counterfactual.py`, `eval/counterfactual-schema.json`, `tests/fixtures/counterfactual/`, `tests/test_eval_counterfactual.py`, `.github/workflows/ci.yml` | shipped (issue #157) |
