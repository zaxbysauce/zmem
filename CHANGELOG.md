...
- **memory** (issue #116): the shared hook body's decision line reports the
  two formerly conflated numbers as distinct labeled fields —
  `rendered_estimate=`, `admission_budget=`, plus byte-stable
  `budget_dropped=` / `budget_truncated=` / `budget_dropped_protected=`
  counts — while the pinned `tokens=a/b` shape stays (the separate
  `zmem-session-start.sh` bash writer keeps its legacy tokens-only line).
  The `--for-injection` envelope gains `budget_dropped` /
...
failing Codex SessionStart hook, add a per-host injection canary (#108).

- **Diagnosis (dated 2026-09-05, codex-cli 0.153.0).** The `hook:
  SessionStart Failed` line under `codex exec` belongs to the operator's
  unrelated `code-review-graph` hook in `~/.codex/hooks.json` (binary absent,
  POSIX-authored command), NOT to zmem — bisected by renaming that file away.
  The re-test the issue mandates then exposed the real zmem defect below.
- **Fix: the Codex manifest hooks path needs the `./` prefix.** codex-cli
  0.153.0 silently ignores a plugin's hooks whose manifest path lacks `./`
  (its own debug log: `ignoring hooks: path must start with ./ relative to
  plugin root`); the same zero-injection symptom was observed on 0.152.1
  (issue #108, 2026-09-02). Every `codex exec` run loaded zero zmem hooks,
  wrote zero bg-log lines, and injected nothing. `.codex-plugin/plugin.json` now declares
  `./hooks/hooks.codex.json` (the Claude manifest already complied; the ZCode
  manifest keeps the unprefixed form ZCode loads today). Pinned by
  `tests/test_codex_manifest_contract.py`; `tests/test_codex_adapter.js`
  updated alongside. After a refresh, re-grant hook trust once (interactive
  `/hooks` browser) — until then the canary correctly reports
  `hook-not-fired` on Codex.
- **`scripts/host_canary.py` — the per-host injection canary.**
  `--host claude|codex|zcode|hermes`; `--self-test` drives the real hook chain
  deterministically (no host binary), live mode runs a minimal non-interactive
  host session (absent binary → `verdict=skip`, exit 0). Seeds one
  recognizable row into an isolated scratch store — ambient
  `ZMEM_STORE`/`ZMEM_DATA`/plugin-data vars are stripped by exact name so the
  real store is never touched (`--probe-store-path` proves the resolution) —
  then asserts a fresh `zmem-hook status=... reason=...` decision line whose
  `ids=[...]` carries the seeded row, and prints the served-tree drift status
  from #107. Failure modes: `hook-not-fired` (exit 2), `no-row-id` (exit 3).
  Covered by `tests/test_host_canary.py`.
- **CI** runs the canary in `--self-test` mode for claude + codex on both
  matrix legs; **README** documents it as the post-install verification step
  and retires the stale "session-start lines carry no reason=" claim
  (verified 2026-09-05: they do, #89/#114-era).
- **doctor** now requires the Codex plugin surface (`.codex-plugin/plugin.json`
