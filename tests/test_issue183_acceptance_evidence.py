...
class Issue183AcceptanceHostEvidenceTest(unittest.TestCase):
    def test_ac9_detached_fail_open_retention_cli_cadence_and_edit_data(self):
        ...
        with tempfile.TemporaryDirectory(prefix="zmem-183-ac9-") as td:
            ...
            self.assertEqual([item["id"] for item in listing], [row["id"] for row in rows])
            shown = _run(
                scratch,
                "evidence",
                "show",
            ...
            with patch.dict(os.environ, env, clear=False), patch.object(
                ...
                from storelib.evidence import sweep_evidence

                retention_conn = sqlite3.connect(env["ZMEM_STORE"])
                with retention_conn:
                    retention = sweep_evidence(retention_conn, now_ts="2026-09-10T00:00:00Z")
                retention_conn.close()
            self.assertEqual(retention["expired"], 1)
            with patch.dict(os.environ, env, clear=False), patch.object(
                sys,
                "path",
                [str(ROOT / "skills" / "memory" / "scripts"), *sys.path],
            ...
            self.assertEqual(report["evidence_expired"], 0)
