...
class CliAllowedGate(unittest.TestCase):
    ...
    def test_passive_matrix(self):
        ...
        self.assertTrue(cli_allowed(no_bump=True, no_hybrid=False,
                                    for_injection=True))
        self.assertFalse(cli_allowed(no_bump=False, no_hybrid=False,
                                     for_injection=True))
        self.assertTrue(cli_allowed(no_bump=True, no_hybrid=True,
                                    for_injection=True))
        ...
        for on, nb, nh, want in matrix:
            ...
            self.assertEqual(
                ...
                want,
                f"explicit: env_on={on} no_bump={nb} no_hybrid={nh}")
            self.assertEqual(
                cli_allowed(no_bump=nb, no_hybrid=nh), want,
                f"omitted: env_on={on} no_bump={nb} no_hybrid={nh}")
...
TOKENIZER_SHA256 = (
    "caef352a8affbe14f5235b5487dd766f2133ddfb70c2d59bfca5734496051cd7")
...
class CrossEncoderProfileAndBudgetTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._saved = {k: os.environ.get(k) for k in (
            ...
            "ZMEM_MODEL_AUTODOWNLOAD", ENABLE, MODEL_ENV,
            "ZMEM_CROSS_ENCODER_MODEL_URL", "ZMEM_CROSS_ENCODER_PASSIVE",
            "ZMEM_CROSS_ENCODER_SHADOW", "ZMEM_CROSS_ENCODER_BUDGET_MS",
            "ZMEM_EMBED_PROFILE")}
        ...
        os.environ["ZMEM_MODEL_AUTODOWNLOAD"] = "0"
        os.environ["ZMEM_EMBED_PROFILE"] = "fake"
        for k in (ENABLE, MODEL_ENV, "ZMEM_CROSS_ENCODER_MODEL_URL",
                  "ZMEM_CROSS_ENCODER_PASSIVE", "ZMEM_CROSS_ENCODER_SHADOW",
                  "ZMEM_CROSS_ENCODER_BUDGET_MS"):
            os.environ.pop(k, None)
        sys.path.insert(0, str(SCRIPTS))
        for m in list(sys.modules):
            if m.startswith("storelib") or m in ("store", "embeddings",
    ...
    def test_default_profile_has_nonempty_checksum(self):
        ...
        self.assertEqual(str(ctx.exception),
                         "unknown cross-encoder profile: no-such-profile")

    def test_fixture_hashes(self):
        model = (REPO_ROOT / "tests" / "fixtures" / "cross_encoder"
                 / "mini_pair_scorer.onnx")
        tok = (REPO_ROOT / "tests" / "fixtures" / "cross_encoder"
               / "tokenizer.json")
        self.assertEqual(model.stat().st_size, 128)
        self.assertEqual(
            hashlib.sha256(model.read_bytes()).hexdigest(), PROFILE_SHA256)
        ...
        with mock.patch("urllib.request.urlopen", _spy):
            for value in (None, "0", "yes", "true"):
                ...
                self.assertEqual(attempts, [],
                                 f"download attempted with "
                                 f"AUTODOWNLOAD={value!r}")
                self.assertIn(
    ...
    def test_checksum_mismatch_fails_open(self):
        ...
        models_dir.mkdir(parents=True, exist_ok=True)
        dest = profiles.profile_model_path(
            models_dir, profiles.resolve_profile())
        dest.write_bytes(b"not the pinned model bytes")
        tok = models_dir / "tokenizer.json"
        tok.write_bytes(
            (REPO_ROOT / "tests" / "fixtures" / "cross_encoder"
             / "tokenizer.json").read_bytes())
        ce.set_scorer(None)
        self.assertIsNone(ce._local_scorer())
    ...
    def test_budget_exhaustion_preserves_order(self):
        ...
        try:
            ...
            reason_lines = [line for line in err.getvalue().splitlines()
        ...
        add_memory(conn, namespace="user:issue-125", type_="fact",
        ...
        out, err = io.StringIO(), io.StringIO()
        old_argv = list(sys.argv)
        sys.argv = ["store.py", "recall", "--query", "budget",
...
class CrossEncoderShadowAndFinalSetTests(unittest.TestCase):
    ...
    def _shadow_rows(self):
        return [{"id": SHADOW_IDS[0], "content": "alpha row one"},
    ...
    def test_shadow_logs_deltas_without_reordering(self):
        ...
        try:
            ...
            log_path = (Path(os.environ["ZMEM_DATA"])
                        / "cross-encoder-shadow.jsonl")
    ...
    def test_passive_path_scores_only_final_rows(self):
        ...
        from storelib import recall as recall_module
        from storelib.cross_encoder import set_scorer
        admitted = [{"id": "00000000-0000-4000-8000-%010d" % (i + 1),
                     "content": "issue 125 admitted candidate row %02d" % i}
                    for i in range(10)]
        deep_pool = [{"id": "00000000-0000-4000-8000-%010d" % (i + 11),
                      "content": "issue 125 deep pool row %02d" % i}
                     for i in range(40)]
        scored_texts = []
        set_scorer(lambda q, texts: (
            scored_texts.extend(texts),
            [float(len(texts) - i) for i in range(len(texts))])[1])
        try:
            os.environ[ENABLE] = "1"
            os.environ["ZMEM_CROSS_ENCODER_PASSIVE"] = "1"
            os.environ["ZMEM_CROSS_ENCODER_SHADOW"] = "1"
            ...
            self.assertEqual(collections.Counter(scored_texts),
                             collections.Counter(
                                 r["content"] for r in admitted),
                             "the scorer must see exactly the 10 final rows")
            self.assertEqual(len(scored_texts), 10)
            pool_hits = [t for t in scored_texts
                         if t in {r["content"] for r in deep_pool}]
            self.assertEqual(pool_hits, [],
                             "deep-pool rows must never be evaluated")
            self.assertEqual([r["id"] for r in out],
                             [r["id"] for r in admitted])
            for r in out:
                self.assertNotIn("score", r)
        finally:
            set_scorer(None)
    ...
    def _seed(self):
        ...
        conn = _connect()
        _prepare_store(conn)
        admit = ["gamma receptor alignment note one",
                 "gamma receptor alignment note two"]
        lowconf = ["gamma receptor untrusted whisper one",
        ...
        for content in lowconf:
            add_memory(conn, namespace="user:issue-125", type_="fact",
                       content=content, confidence=0.05)
        conn.close()
        ...
        if isinstance(value, dict):
            for k, v in value.items():
                if k == "content" and isinstance(v, str):
                    ...
                    found.extend(
                        CrossEncoderShadowAndFinalSetTests._walk_contents(v))
        elif isinstance(value, list):
            for item in value:
                found.extend(
                    CrossEncoderShadowAndFinalSetTests._walk_contents(item))
        return found
    ...
    def test_production_passive_wiring_scores_final_set(self):
        ...
        try:
            ...
            code, out_text, err_text = self._drive_ce_cli(
                ["recall", "--query", "gamma receptor",
                 "--namespace", "user:issue-125", "--limit", "5",
                 "--include-global", "--global-limit", "3", "--no-bump",
                 "--for-injection", "--json",
                 "--session-id", "sess-125-wiring",
                 "--moment", "user_prompt"])
            self.assertEqual(code, 0, err_text)
            payload = json.loads(out_text)
            contents = self._walk_contents(payload)
            for row in admit:
                self.assertIn(row, scored_texts,
                              "production chain never scored an admitted row")
                self.assertIn(row, contents)
            for row in lowconf:
                self.assertNotIn(row, scored_texts,
                                 "gate-dropped row reached the scorer")
        ...
        try:
            ...
            code, out_text, err_text = self._drive_ce_cli(
                ["recall", "--query", "gamma receptor",
                 ...
                 "--include-global", "--global-limit", "3", "--no-bump",
                 "--for-injection", "--json",
                 "--session-id", "sess-125-neg",
                 "--moment", "user_prompt"])
            self.assertEqual(code, 0)
            json.loads(out_text)
            self.assertEqual(scored_texts, [],
                             "PASSIVE unset must keep the scorer untouched")
        finally:
            set_scorer(None)
        ...
        try:
            ...
            self.assertEqual(
                [line for line in err.getvalue().splitlines()
                 if "cross-encoder reason=" in line].count(
                     "[zmem] cross-encoder reason=applied"), 1)
        finally:
            set_scorer(None)
    ...
    def test_shadow_write_failure_emits_load_error(self):
        ...
        (Path(os.environ["ZMEM_DATA"])
         / "cross-encoder-shadow.jsonl").mkdir(parents=True)
        set_scorer(lambda q, texts: [1.0 for _ in texts])
        try:
            with contextlib.redirect_stderr(io.StringIO()) as err:
                out = recall_module.rerank_final_injection_set("query", rows)
    ...
    def test_queryless_recent_selector_passes_rows(self):
        ...
        scored_texts = []
        set_scorer(lambda q, texts: (
            scored_texts.extend(texts), [1.0 for _ in texts])[1])
        try:
            os.environ[ENABLE] = "1"
            os.environ["ZMEM_CROSS_ENCODER_PASSIVE"] = "1"
            os.environ["ZMEM_CROSS_ENCODER_SHADOW"] = "1"
            code, out_text, err_text = self._drive_ce_cli(
                ["recent", "--namespace", "user:issue-125", "--limit", "3",
                 "--include-global", "--global-limit", "2", "--no-bump",
                 "--for-injection", "--json",
                 "--session-id", "sess-125-recent",
